(function (global) {
  ...
  function sortAscending(obj2, obj1) {
  ...
  function resetHeaderLocations() {
    var headers = $(".tocify-header").sort(sortAscending);
  ...
  function search(event) {
    ...
    if (this.value) {
      ...
      $.each(tmpResults, function (index, item) {
        if (item.score >= 0.001) results.push(item);
      });
      ...
      if (results.length) {
        ...
        $.each(results, function (index, item) {
          ...
          if (lastRef) {
            refToHeader(lastRef).insertBefore(refToHeader(itemRef));
          }
          closestHeader.show();
          lastRef = itemRef;
        });
